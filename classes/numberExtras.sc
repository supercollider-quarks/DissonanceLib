...
//			bk = (26.81 / (1 + (1960 / this)) - 0.53); // just slightly different formulation
	...
	roughness {|f2| var dif = absdif(this.asBark, f2.asBark);
		^(4 * dif * (exp(1-(4 * dif))))
	}
	...
	highestPower {| minHarmonicity = 1, pitchRange = 1|
		...
		if (this == 2) {
			^((pitchRange + (minHarmonicity.reciprocal)) /
				(1 + (256.log / 27.log))).trunc.asInteger
		}{
			^((pitchRange + (minHarmonicity.reciprocal)) /
				(this.indigestibility + (this.log / 2.log))).trunc.asInteger
		}
	}
...
//	}
	...
	ratioToFreq {|fundFreq = 1|
		if ( (this.size == 2) and: (this[0].isNumber) )
			{
				^((this[0]/this[1]) * fundFreq)
			}{
				^this.collect{|x| x.ratioToFreq * fundFreq }
			}
	}

	invRatioToFreq {|fundFreq = 1|
		if ( (this.size == 2) and: (this[0].isNumber) )
		{
			^((this[1]/this[0]) * fundFreq)
		}{
			^this.collect{|x| x.invRatioToFreq(fundFreq)}
		}
	}
...
//	several methods designed to work with arrays of pairs (either rationals or [', spl]):
	...
	// Leonhard Euler's Gradus Suavitatis (1739, 'degree of sweetness'), arrays of rational pairs [p,q]:
	...
	// this is an array of freqs
	chordRoughness{
		var dissonance = 0, roughness;
		this.size.do{|i|
			this.size.do{|j|
				if (i < j) {
					roughness = this[i].roughness(this[j]);
	...
	// this is an array of freqs with an amps argument which is an array of amps
	chordDissonance{|amps, calib = 100, compensate = true, gradient = 20|
		var sones, dissonance = 0, roughness;
		if (compensate) {
		  	sones = [this, [this, amps.ampdb+calib].flop.compensateMasking(gradient)].
		  		flop.asSone(0);
		}{
			sones = [this, amps.ampdb].flop.asSone(calib);
		};
		this.size.do{|i|
			this.size.do{|j|
				if (i < j) {
					roughness = sqrt(sones[i] * sones[j]) * this[i].roughness(this[j]);
					dissonance = dissonance + roughness;
				};
			};
		};
...
// the following are methods for SimpleNumber made to work in SequenceableCollection:
	...
	centsToName {|tolerance = 12, restore = true|
		...
		this.do{|int, indx|
		  postf("The following intervals are close to %¢ by +/- %¢:\n", int, tolerance);
		  g[indx].do{|x|
...
/*							}*/
		...
		if (post) {
			postf("Each scale degree is close to the following intervals by +/-%¢:\n", tolerance);
...
// jsl: 2005-2008
/*
	TO DO: indispensability for meters
*/
